ВЕРСИЯ 8.16.7 от 17.03.26
Исправлена ошибка в WEB интерфейсе, когда сохранение на одной странице сбрасывает галочки чекбоксов на других

ВЕРСИЯ 8.16.6 от 15.01.26
Добавлен глвый сенсор на 7.5м
Максимальные дистанции трех сенсоров - 5.9м, 4.9м и 7.4м

ВЕРСИЯ 8.16.3 от 26.09.25
Поле название устройства пенесено наверх под тип сенсора

ВЕРСИЯ 8.16.3 от 24.09.25
Исправлена ошибка в WEB интерфейсе по интервалам Реле 1 и Реле 2

ВЕРСИЯ 8.15.2 от 11.06.25
1. В веб интерфейсе добавлена возможность настройки подсветки всех режимов 

ВЕРСИЯ 8.15.1 от 06.05.25
1. Добавлен выбор типа сенсора в WEB-интерфейсе 

ВЕРСИЯ 8.14.3 от 28.04.2025 (исправления)
1. Исправлена ошибка сохранения 1-й страницы
2. Исправлена ошибка перехода на главную после сохранения. Сейчас остается текущая страница
3. Картинка WiFi поправлена
4.Если поднести магнит после 2 сек горит зеленым
5. Если 5 раз поднести магнит горит оранжевым во время сохранения 

ВЕРСИЯ 8.14.3 от 27.04.2025
1. Главная страница настроек вебинтерфейса разбира на три. Оптимизация вебинтерфейса
2. При начале калибровки (геркон) лента мигает белым, затем жельым и при начале калибровки горит белым
3. Если поднесении магнита 7 раз с паузой менее 2 сек сбрасывается имя точки доступа
   (Работает не четко. если между поднесениям к магниту были переключения свободно/занято, отправки на сервер, загрузки страницы и пр. счетчик нажатий сбрасывается и нужно все заново или бывают пропуски нажатий)

ВЕРСИЯ 8.8.14 от 23.04.2025
1. Добавлен режим - импульс открытия/закрытия
2. Ссылки "Прошивка, поумолчанию и т.д." зпменены на кнопки
3. Немного доработан WEB-интерфейс


ВЕРСИЯ 8.8.2 от 19.02.25
1. При измерении дистанции добавлен таймаут 
#define TM_ECHO                100000        //Таймаут pulseIn в мкс
2. Добавлен режим работы реле "Импульс реверсный" (при переходе на красный свет)

ВЕРСИЯ 8.8.1 от 12.12.24
1. Правки WEB-интерфейса по блокам (задержки вкдючения пернесены в настройки реле, 
   задержка между циклами в блок "если датчик не видит расстояния")
2. Зодержка включение и выключения реле настраивается и обрабатывается для Реле 1 и Реле 2 раздельно
3. Добавлен "Цикличный режим" для каждого реле с настрокой паузы
4. Режим реле 2 по умолчанию On/OFF
5. Режим "Инверсия выхода" срабатывает сразу
6. Реле 1 и 2 поменял местами пины. PinController = Реле 1, PinRelay = Реле 2
7. Картинки оптимизированы на 4 цвета
8. Во время загрузки WEB-страницы блокируется измерение расстояния


ВЕРСИЯ 8.8.0 от 01.12.24
1. Исправлена ошибка радуга + NAN. Если после при загрузке датчик не видит расстояние, то цвета всегда красный + розовый
2. По умолчанию после прошивки режим раздачи WiFi всегда
   EA_Config.isWiFiAlways    = true;
3. По умолчанию после прошивки нет подключения к crm.moscow
   EA_Config.isSendCrmMoscow = false;
4. Дистанция с кнопкой обновления показывается в отделном фрейме (<IFRAME>)
   При нажатии на кнопку обновляется только текущий фрейм, а не вя страница
5. Добавлены блоки управления режимами реле1 и реле2 (Pinrelay и PinController)
   Режим работы обычный или эмуляция кнопки
   Длительность нажатия кнопки, сек при эмуляции
   Инверсия выхода
...
6. В настройках отправки на сервер задан интервал отправки и поаторной поаытки при ошибке страницы

   По умолчанию
   EA_Config.TM_HTTP_SEND         = 1000; //01.12.24 Таймаут отправки на сервер
   EA_Config.TM_HTTP_RETRY_ERROR  = 10; //01.12.24 Повторная попытка, если при отправке не вернулся статус 200  
   
7. После кнопки "Сброс к заводским настройкам" всегда перезагрузка

8. После обнуленя, прошивки и сроса настроек автокалибровка только после первой перезагрузки (была после двух)

9. Оптимизированы иконки в WEB-интерфейсе (размер уменьшин раза в 3)

   
   
ВЕРСИЯ 5.0.7 16.08.2024
В регулярной отправке HTTP на сервер добавлен статус BTN=

ВЕРСИЯ 5.0.7 21.07.2024
Добавлен счетчик перезагрузок 
Первые две перезагрузки автокалибровка + режим постоянной раздачи
Сброс WiFi ли зброс к заводским настройкам обнуляет счетчик

ВЕРСИЯ 5.0.7 16.07.2024
1. Цвета светодиодный ленты задаются #RRGGBB в файле Config.h (Стр 149-160)
2. В WEB-интерфейсе добавлена возможность выбора: цвет "свободно", цвет "занято", моргание "свободно и его цвет
3. В WEB интерфейсе добавлен выбор яркости ленты 0-10


ВЕРСИЯ 5.0.6 06.04.2024
1. Работа с временем переведена на TimeLib. Неправильное время теперь 30.08.2077
2. Исправлена ошибка обработки пустого значения
3. Функция автокалибровки полностью переписана (калибрует до тех пор пока значение перестает колебаться)
   Если ошибка калибровки, мигает ораньжевым, если все нормально - записывает и мигает белым
4. В WEB интерфейсе все кнопки "Save"
